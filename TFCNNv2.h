...
static inline float lookup_derivative(const float* table, const uint table_size, const float f, const float fn, const float min, const float max)
{
    // table_size should be passed with a -1

    // if(f <= min){return table[0];} else if (f >= max){return table[table_size];}
    // const uint i = (uint)(table_size * fn);
    // const float fx = (fn - table[i]) * fnss;
    // return table[i] * (1.f - fx) + table[i+1] * fx;

    if(f <= min) {return table[0];} else if (f >= max) {return table[table_size];}
    const float fi = (float)table_size * fn;
    const uint  i  = (uint)fi;
    const float fd = fi - floor(fi);
    //printf("%u %f\n", i, fd);
    return table[i] * (1.f - fd) + table[i+1] * fd;
    //return sinusoid_derivative_table[i];
}
...
static inline float geluDerivative(const float x)
{
    ...
    return gelu_derivative_table[i] * (1.f - fx) + gelu_derivative_table[i+1] * fx;
}
