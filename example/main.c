...
--------------------------------------------------
    James William Fletcher (james@voxdsp.com)
        October 2020 - TFCNNv2 - Example Demo
--------------------------------------------------
...
void rndBest()
{
    ...
    while(1)
    {
        ...
        while(fv < min || fv > max) //we want random string to fail at-least some percent of the time more than 50% preferably
        {
            ...
            printf("Dropout:       %f\n", net.dropout);
            printf("WDropout:      %f\n", net.wdropout);
            printf("Dropout Decay: %f\n", net.dropout_decay);
            if(net.optimiser == OPTIM_ADAGRAD || net.optimiser == OPTIM_RMSPROP)
                printf("Epsilon:       %f\n", net.epsilon);
            if(net.activator == LEAKYRELU || net.activator == ELU)
                printf("ELUAlpha:      %f\n", net.elualpha);
...
void bestSetting(const float min)
{
    ...
    while(1)
    {
        ...
        if(f != NULL)
        {
            ...
            fprintf(f, "L-Rate: %f\n", a3/count);
            fprintf(f, "Dropout: %f\n", a4/count);
            fprintf(f, "WDropout: %f\n", a2/count);
            fprintf(f, "Dropout Decay: %f\n", a9/count);
            if(a5 != 0)
                fprintf(f, "Momentum: %f / %u\n", a5/c1, c1);
            if(a6 != 0)
                fprintf(f, "RMS Alpha: %f / %u\n", a6/c2, c2);
            if(a7 != 0)
                fprintf(f, "Epsilon: %f / %u\n", a7/c3, c3);
            if(a8 != 0)
                fprintf(f, "ELU Alpha: %f / %u\n", a8/c4, c4);
            fprintf(f, "\n");
            for(uint i = 0; i < 5; i++)
...
{
    // init discriminator
    const int ret = createNetwork(&net, 2, DIGEST_SIZE, HIDDEN_LAYERS, HIDDEN_SIZE, 1);
    if(ret < 0)
    {
    ...
    if(argc == 2)
    {
        if(strcmp(argv[1], "retrain") == 0)
        {
            ...
            const uint l = uRand(0, net.num_layers-2);
            const uint lu = uRand(0, net.num_layerunits-1);
            const uint a = uRand(0, net.layer[l][lu].weights-1);
            const uint b = uRand(0, net.layer[l][lu].weights-1);
            const uint c = uRand(0, net.layer[l][lu].weights-1);
            printf("RWV: [%u][%u]\n", l, lu);
        ...
        if(strcmp(argv[1], "check") == 0)
        {
            ...
            struct stat st;
            const int sr = stat("network.dat", &st);
